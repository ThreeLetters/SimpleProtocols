...
module.exports = function (obj, data, name, type, lvl, opt) {
    ...
    var useOneBitFlag = false;
    var useBreak = false;
    var breakArray = data.breakArrays[data.breakArrays.length - 1];
    ...
    if (dtobj[split[1]]) {
        ...
        if (breakArray && !breakArray.useBreak && maxScramble == 255 && mindiff != 0) {
            breakArray.useBreak = true;
            useBreak = true;
            breakArray.breakKey = max + 1;
            mindiff--;
            diff++

        }
        if (breakArray && !breakArray.useOneBitFlag) {
            if (mindiff > diff) {
                mindiff = Math.floor((mindiff - diff) / 2);
                useOneBitFlag = true;
                breakArray.useOneBitFlag = true;
            }
        }
        ...
        if (opt.scrambleNumbers) {
            var offset = Math.floor(Math.random() * (mindiff + 1));
            min += offset;
        }
        omin = min;
        if (min) {
    ...
    if (opt.scrambleNumbers && scrambleBinary) {
        scrambleAmount = (Math.floor(Math.random() * (maxScramble - minScramble + 1)) + minScramble);
    ...
    var startSet = set + rounderstart + decimalsStart;
    var endSet = scramble + ")" + rounderend + ";";
    if (useBreak) {
        breakArray.getCode = get;
        breakArray.breakKey = (((breakArray.breakKey + omin) << (useOneBitFlag ? 1 : 0)) ^ scrambleAmount) >>> 0;
        get = "b" + breakArray.lvl;
        endSet = endSet.substring(0, endSet.length - 2) + ",c" + breakArray.lvl + ");"
    }
    if (useOneBitFlag) {
        get = `((a${breakArray.lvl}=${get + scramble}) >> 1)`;
        startSet += "((";
        endSet = `<< 1) | (i${breakArray.lvl} + 1 < len${breakArray.lvl}))` + endSet;
    } else {
        get = "(" + get + scramble + ")";
    }
    get = rounderstart + get + rounderend;

    get = decimalsStart + get + off + intToDecimals;
    switch (type) {
        case 0: // no object
            code = "data" + lvl2 + "=" + get + ";";
            code2 = startSet + "(data" + lvl2 + decimalsToInt + off2 + ")" + endSet;
        ...
        case 1: // object
            ...
            code2 = startSet + "(data" + lvl + "." + name + decimalsToInt + off2 + ")" + endSet;
