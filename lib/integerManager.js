...
module.exports = function (obj, data, name, type, lvl, opt) {
    ...
    if (dtobj[split[1]]) {
        ...
        if (breakArray && !breakArray.covered) {
            if (mindiff > diff) {
                mindiff = mindiff - diff;
                useBreak = true;
                breakArray.covered = true;
            }
        }
    ...
    if (opt.scrambleNumbers && scrambleBinary) {
        scramble = "^" + (Math.floor(Math.random() * (maxScramble - minScramble + 1)) + minScramble);
        rounderend = ">>>0)"
        rounderstart = "("
    }
    var decimalsToInt = "",
    ...
    get = decimalsStart + rounderstart + get + rounderend + off + intToDecimals;
