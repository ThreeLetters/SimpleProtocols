...
module.exports = function (object, options) {
    ...
    var data = {
        get: new DataService(), // Code that gets data from binary
        set: new DataService(), // Code that sets data into binary
        count: new DataService(), // Code that counts how many bytes are needed
        byteLen: [], // Combines the # of bytes of certain things (such as an array of numbers), so that a loop is avoided
        index: 0,
        byteSize: [], // Combines coefficients
        canMultiply: [], // If true, then no loop will be created to count, and k*length will happen instead
        if: false,
        breakArrays: []
    };
    ...
    data.get.push("var reader = new Reader(buf, startIndex);");
                ...
                if (data.byteLen[data.index]) {
                    if (ret) {
                        if (typeof obj[0] == "string" && obj[0].substr(0, 6) == "length") {
                            data.count.push("byteLen += " + data.byteLen[data.index] * parseInt(obj[0].split(" ")[1]) + ";");
                        } else {
    ...
    data.count.push("var writer = new Writer(byteLen + (startIndex || 0) + sizeAdd, startIndex);")
    ...
    var get = "function " + options.getname + "(buf, startIndex) {\n" + FastBuffers.NodeJS("reader") + "\n" + getCode + "}";
    var set = "function " + options.setname + "(data1, startIndex, sizeAdd) {\n" + FastBuffers.NodeJS("writer") + "\n" + setCode + "}";
    var getBrowser = "function " + options.getname + "(buf, startIndex) {\n" + FastBuffers.Browser("reader") + "\n" + getCode + "}";
    var setBrowser = "function " + options.setname + "(data1, startIndex, sizeAdd) {" + FastBuffers.Browser("writer") + "\n" + setCode + "}";
