...
module.exports = function (object, options) {
    ...
    var parse = function (obj, name, type) {
        ...
        if (typeof obj == "object") {
            ...
            if (obj.if && obj.then && obj.else) return ObjectParse.conditional(obj, data, parse, type, name, lvl, options, dtlvl);
            if (type == 1) {
            ...
            if (Array.isArray(obj)) {
                ...
                if (typeof obj[0] == "string" && obj[0].substr(0, 6) == "length") ObjectParse.fixedArray(obj, data, parse, lvl, options);
                else ObjectParse.array(obj, data, parse, lvl, options);


            } else {


                ObjectParse.object(obj, data, parse, lvl, type, name, options);
            }
            lvl--;
            var lvl2 = lvl + 1;
            //   if (!skip) {
    ...
    var get = "function get(buf) {" + FastBuffers.NodeJS("reader") + "\n" + getCode + "}";
    var set = "function set(data1) {" + FastBuffers.NodeJS("writer") + "\n" + setCode + "}";
    var getBrowser = "function get(buf) {" + FastBuffers.Browser("reader") + "\n" + getCode + "}";
    var setBrowser = "function set(data1) {" + FastBuffers.Browser("writer") + "\n" + setCode + "}";
    return {
        get: get,
        set: set,
        getb: getBrowser,
