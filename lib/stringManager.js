...
module.exports = function (obj, data, name, type, lvl, opt) {
    ...
    if (opt.maskStrings) {
        ...
        var minm = opt.maskLength[0],
            maxm = opt.maskLength[1];

        var maskLength = Math.floor(Math.random() * (maxm - minm + 1)) + minm;
        var mask = [];
        for (var i = 0; i < maskLength; i++) {
            mask.push(Math.floor(Math.random() * (max + 1)));
        }
        var maskString = JSON.stringify(mask);
        get += maskString

        maskCodeSet = ", " + maskString;
    ...
    switch (type) {
        case 0: // no object
            ...
            code2 = set + func + "data" + lvl2 + func2 + maskCodeSet + ")"
            code3 = "byteLen += " + len + " + data" + lvl2 + ".length * " + len + ";";
            break;
        case 1: // object
            code = "data" + lvl + "." + name + "=" + func + get + func2 + ";";
            code2 = set + func + "data" + lvl + "." + name + func2 + maskCodeSet + ");";
